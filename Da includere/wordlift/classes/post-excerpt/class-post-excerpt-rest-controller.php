...
class Post_Excerpt_Rest_Controller {
	...
	public static function get_post_excerpt_from_remote_server( $post_id, $post_body ) {
		// MODIFIED: Use local excerpt generation instead of remote API
		$excerpt = self::generate_local_excerpt( $post_body );
	...
	public static function save_response_to_meta_on_success( $post_id, $post_body, $response ) {
		// MODIFIED: Handle both local and remote responses
		$post_excerpt = null;
		
		// Check if response is a Response object or array
		if ( is_object( $response ) && method_exists( $response, 'get_body' ) ) {
			...
			// Local mode: body is directly the excerpt string
			$post_excerpt = $response['body'];
		}
