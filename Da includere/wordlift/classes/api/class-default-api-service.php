...
class Default_Api_Service implements Api_Service, Api_Service_Ext {
	...
	public function request( $method, $path, $headers = array(), $body = null, $timeout = null, $user_agent = null, $args = array() ) {
		...
		// MODIFIED: Block calls to WordLift API in local-only mode
		$request_url = $this->base_url . $path;
		
		// If base_url is empty or points to WordLift API, block the request
		if ( empty( $this->base_url ) || strpos( $this->base_url, 'wordlift.io' ) !== false || strpos( $this->base_url, 'wordlift.it' ) !== false ) {
			$this->log->warn( "Local mode: Blocked API call to WordLift service: $request_url" );
			// Return error response
			return new Response( new \WP_Error( 'local_mode', 'WordLift API calls are disabled in local-only mode' ) );
		}
