...
class Wordlift_Key_Validation_Service {
	 ...
	 * @since 3.9.0
	 * MODIFIED: Local-only mode - always returns valid response without API call
	 */
	public function get_account_info( $key ) {

		$this->log->debug( 'Local mode: Skipping key validation (local-only mode)' );

		// Return mock valid response for local-only mode
		$mock_response = array(
			'response' => array(
				'code' => 200,
			),
		 ...
		 */
		do_action( 'wl_key_validation_response', $mock_response );
	 ...
	 *
	 * @return bool
	 * MODIFIED: Local-only mode - always returns true
	 */
	public function is_key_valid( $key ) {
		// In local-only mode, always return true
		return true;
	}
	 ...
	 *
	 * @since 3.9.0
	 * MODIFIED: Local-only mode - always returns valid
	 */
	public function validate_key() {
		...
		// In local-only mode, always return valid
		wp_send_json_success(
			array(
				'valid'   => true,
				'message' => __( 'Local mode: Key validation skipped', 'wordlift' ),
			)
		);
	}
	 ...
	 * MODIFIED: Local-only mode - only updates blog URL, doesn't remove key
	 */
	public function wl_load_plugin() {
		...
		if ( ! $wl_blog_url ) {
			...
			update_option( '_wl_blog_url', $home_url, true );
			// In local-only mode, don't remove the key when URL changes
		}
	}
