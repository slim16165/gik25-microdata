...
class Wordlift_Configuration_Service {
	...
	public function get_remote_dataset_uri( $key ) {
		...
		$this->log->trace( 'Local mode: Using local dataset URI instead of remote...' );

		// MODIFIED: In local-only mode, use local dataset URI instead of calling API
		$home_url = get_option( 'home' );
		$site_url = apply_filters( 'wl_production_site_url', untrailingslashit( $home_url ) );
		
		// Generate local dataset URI based on site URL
		$local_dataset_uri = $site_url . '/dataset/';
		$this->set_dataset_uri( $local_dataset_uri );
		$this->set_package_type( 'local' );
		
		$this->log->info( "Local mode: Dataset URI set to $local_dataset_uri" );
		
		return;
		
		// Original code below (disabled for local mode):
		/*
		if ( empty( $key ) ) {
