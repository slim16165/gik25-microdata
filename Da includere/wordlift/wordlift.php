...
define( 'WORDLIFT_VERSION', '3.54.6' );
...
function deactivate_wordlift() {
	 ...
	 * DISABLED: Key validation removed for local-only mode
	 */
	flush_rewrite_rules();
...
// MODIFIED: Force local-only mode - disable cloud dataset sync but keep local functionality
add_filter( 'wl_feature__enable__dataset', '__return_true', 5 ); // Keep dataset enabled for local entities
add_filter( 'wl_feature__enable__analysis', '__return_true', 5 ); // Keep analysis enabled (uses local service)
...
// DISABLED: Redeem code module requires WordLift cloud service
