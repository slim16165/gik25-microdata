...
//if(in_array( 'plugin-load-filter/plugin-load-filter.php', (array)$active_plugins))
//{
		...
		function FilterActivePlugins()
		{
			...
			if (!empty($_SERVER['REQUEST_URI']))
			{
				...
				if ($urlkey !== false && is_string($urlkey))
				{
					...
					{
						$unload = false;
						$urlDaFiltrare = $pluginAttivoCorrente;

						if (!empty($filter['plfurlkey'][$urlkey]['plugins']))
						{
							if (false !== strpos($filter['plfurlkey'][$urlkey]['plugins'], $urlDaFiltrare))
								$unload = true;
						}
						if (!$unload)
						{
...
//				add_action('parse_request', array(&$this, 'parse_request'));
			...
			//Unserialize pluginAttivi_string only if it was serialized.
			$pluginAttivi = maybe_unserialize($pluginAttivi_string);
