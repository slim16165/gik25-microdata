 ...
 * @author wamania
 *
 * Modified somewhat to allow stemming of de-accented words.
 * Deaccent processing was needed for accounting for cases where suggestion searches were performed with keywords made of ASCII letters.
 * 
 */
class French extends Stem
{
    ...
    protected static $deaccent_stemm = false;

    /**
     * {@inheritdoc}
     */
    public function Stem($word, $deaccent = false, $ignore_cache = false)
    {
        // we do ALL in UTF-8
        if (! Utf8::check($word)) {
    ...
    private function step1()
    {
        ...
        if ( ($position = $this->search(array('ités', 'ité'))) !== false ||
              self::$deaccent_stemm && ($position = $this->search(array('ites', 'ite'))) !== false) 
            {
    ...
    private function step2a()
    {
        ...
        $suffixes = array(
            ...
            'issent', 'isses', 'issez', 'isse', 'issiez', 'issions', 'issons', 'is', 'it', 'i');

        if(self::$deaccent_stemm){
            $suffixes = array_merge(array('imes'));
        }

        if ( ($position = $this->searchIfInRv($suffixes)) !== false) {
    ...
    private function step2b()
    {
        $suffixes = array(
            ...
            'erions', 'erons', 'eront', 'era', 'er', 'iez', 'ez','és', 'ée', 'é');

        if(self::$deaccent_stemm){
            $suffixes = array_merge(array('ees', 'erent', 'es', 'ee'));
        }
    ...
    private function step4()
    {
        ...
        $suffixes = array('ier', 'ière', 'Ier', 'Ière');

        if(self::$deaccent_stemm){
            $suffixes = array_merge(array('iere'));
        }

        // ier   ière   Ier   Ière
        //      replace with i
        if ( ($this->searchIfInRv($suffixes)) !== false) {
            $regx = '#(' . implode('|', $suffixes) . ')$#u';
            $this->word = preg_replace($regx, 'i', $this->word);
            return true;
        }
