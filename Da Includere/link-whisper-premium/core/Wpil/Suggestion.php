...
//            $unique_keywords = self::getPostUniqueKeywords($target_keywords, $process_key);
            ...
            if(!empty($target_keywords)){
                ...
                foreach($target_keywords as $target_keyword){
                    ...
                    if($in_unstemmed || $in_stemmed || ($in_normalized && !empty($normalized))) 
                    {
                        ...
                        if($in_unstemmed){
                            $pos = mb_strpos(Wpil_Word::strtolower($phrase->text), Wpil_Word::strtolower($target_keyword->keywords));
                        ...
                        if($in_normalized){
                            $pos = mb_strpos($normalized, $target_keyword->normalized);
                        ...
                        if($in_normalized){
                            ...
                            $before_words = mb_substr($normalized, 0, mb_strpos($normalized, $target_keyword->normalized));
    ...
    public static function getPhrases($content, $with_links = false, $word_segments = array(), $single_words = false, $ignore_text = array())
    {
        ...
        if(false !== strpos($content, '<pre')){
            $content = mb_ereg_replace('<pre(?:[^>]*)>(.*?)<\/pre>', '', $content);
        }
        ...
        $ignore_text .= "(?<![[:alpha:]<>-_1-9])(?:[A-Za-z]\.){2,20}(?![[:alpha:]<>-_1-9])"; // include a regex for searching common abbreviations
    ...
    public static function removeIgnoredContentTags($content = ''){
        ...
        if(!empty($tag_regex)){
            ...
            $content = mb_ereg_replace($tag_regex, '', $content);
        }
    ...
    public static function getTitleWords($post, $target = null, $keyword = null, $count = null, $process_key = 0)
    {
        ...
        if (!is_null($target)) {
            ...
            if(empty($post_ids) && !is_array($post_ids)){
                ...
                $statuses_query = Wpil_Query::postStatuses();
                $post_ids = $wpdb->get_col("SELECT ID FROM {$wpdb->prefix}posts WHERE 1=1 $exclude AND post_type IN ('{$post_types}') $statuses_query {$blood_relative} {$age_string} " . $include);
...
error_log(print_r($post, true));
    ...
    public static function getInboundSuggestedPosts($keyword, $excluded_posts)
    {
        ...
        $selected_terms .= (isset($term_taxonomy_ids[0])) ? " AND ID in (select object_id from {$wpdb->prefix}term_relationships where term_taxonomy_id in (" . implode(',', $term_taxonomy_ids[0]) . ")) " : "";
        $selected_terms .= (isset($term_taxonomy_ids[1])) ? " AND ID in (select object_id from {$wpdb->prefix}term_relationships where term_taxonomy_id in (" . implode(',', $term_taxonomy_ids[1]) . ")) " : "";
        ...
        if(!empty($same_language_posts) && $multi_lang){
            ...
            foreach($id_batches as $batch){
                ...
                $batch_ids = $wpdb->get_col("SELECT `ID` FROM {$wpdb->posts} WHERE post_type IN ('{$post_types}') $statuses_query AND ID NOT IN ({$excluded_posts}) {$age_query} {$selected_terms} {$post_content} $include ORDER BY ID DESC");
            ...
            $posts = $wpdb->get_col("SELECT `ID` FROM {$wpdb->posts} WHERE post_type IN ('{$post_types}') $statuses_query AND ID NOT IN ({$excluded_posts}) {$age_query} {$selected_terms} {$post_content} {$related} ORDER BY ID DESC");
        }
    ...
    public static function getPostsFromAlternateLocations($posts, $keyword, $exclude_ids, $term_taxonomy_ids = array()){
        ...
        if(!empty($builder_meta)){
            ...
            if(!empty($words)){
                ...
                $results = $wpdb->get_col("SELECT DISTINCT m.post_id AS ID FROM {$wpdb->posts} p INNER JOIN {$wpdb->postmeta} m ON p.ID = m.post_id WHERE m.meta_key IN ({$builder_meta}) {$selected_terms} {$age_string} {$post_types_p} {$statuses_query_p} AND ({$meta})");
        ...
        if(defined('WC_PLUGIN_FILE') && in_array('product', $active_post_types)){
            ...
            if(!empty($words)){
                ...
                $results = $wpdb->get_col("SELECT DISTINCT ID FROM {$wpdb->posts} p WHERE p.post_type = 'product' {$selected_terms} {$age_string} AND ({$exerpt})");
        ...
        if(!empty($fields)){
            ...
            if(!empty($words)){
                ...
                $meta = $wpdb->prepare("meta_value LIKE %s {$like}", array_map(array('Wpil_Toolbox', 'esc_like'), $words));
                $results = $wpdb->get_col("SELECT DISTINCT m.post_id AS ID FROM {$wpdb->posts} p INNER JOIN {$wpdb->postmeta} m ON p.ID = m.post_id WHERE m.meta_key IN ('" . implode("', '", $fields) . "') {$selected_terms} {$age_string} {$post_types_p} {$statuses_query_p} AND ({$meta})");
    ...
    public static function getInboundCustomFields($keyword, $term_taxonomy_ids = array())
    {
        ...
        if(count($fields) < 100){
            ...
            $posts_query = $wpdb->get_results("SELECT m.post_id FROM {$wpdb->postmeta} m INNER JOIN {$wpdb->posts} p ON m.post_id = p.ID WHERE p.post_type IN ('{$post_types}') {$age_string} {$selected_terms} $fields $post_content $statuses_query");
