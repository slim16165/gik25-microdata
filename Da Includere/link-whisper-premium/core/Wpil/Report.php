...
//        add_action('wp_ajax_wpil_report_reload', [$this, 'ajax_reload']); // TODO: DELETE IN 2.2.8 if no one misses it
//        add_action('wp_ajax_wpil_back_to_report', [$this, 'ajax_back_to_report']); // TODO: DELETE IN 2.2.8 if no one misses it
...
//                                'wpil_sync_report3'                         => 1,
    ...
    public static function getContentLinks($post, $ignore_post = false)
    {
        ...
        $post_link = Wpil_Link::filter_staging_to_live_domain($post->getLinks()->view);
        $location = 'content';
    ...
    public static function setup_click_details_page()
    {
        ...
        foreach($click_data as $data){
            $time = date(get_option('date_format', 'F d, Y'), strtotime($data->click_date));
    ...
    public static function getData($start = 0, $orderby = '', $order = 'DESC', $search='', $limit=20, $orphaned = false)
    {
        ...
        if ($orderby == 'post_date' || $orderby == 'post_title' || $orderby == 'post_type' || $orderby == 'title_search') {
            ...
            (SELECT p.ID, 'post' AS post_type, p.post_title, p.post_date as `post_date` FROM {$wpdb->posts} p LEFT JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id WHERE 1 = 1 $report_post_ids $statuses_query $ignored_posts AND p.post_type IN ($post_types) $search AND pm.meta_key = 'wpil_sync_report3' AND pm.meta_value = '1'  {$post_filter_query}";

            if ($processing_terms) {
                $taxonomies = Wpil_Settings::getTermTypes();
                $query .= " UNION
                SELECT t.term_id as `ID`, 'term' {$collation} as `post_type`, t.name {$collation} as `post_title`, '1970-01-01 00:00:00' as `post_date`, 'term' as `type
            ...
            $query = "SELECT DISTINCT p.ID, p.post_title {$collation} AS 'post_title', p.post_type {$collation} AS 'post_type', p.post_date as `post_date`, m.meta_value {$collation} AS 'meta_value', 'post' as `type` $title_search  
                        ...
                        WHERE 1 = 1 $report_post_ids $statuses_query $ignored_posts AND p.post_type IN ($post_types) {$post_filter_query} AND m.meta_key LIKE '$orderby' $search";
    ...
    public static function get_dropdown_icons($post = array(), $url = '', $disposition = 'inbound-internal'){
        ...
        $broken = Wpil_Error::checkBrokenLinkFromCache($url);
        if(!empty($broken)){
            $icon = '';
            $icon .= '<div class="wpil_help">';
    ...
    public static function insert_links_into_link_table($post){
        ...
        $insert_query = "INSERT INTO {$links_table} (post_id, target_id, target_type, clean_url, raw_url, host, anchor, internal, has_links, post_type, location, broken_link_scanned) VALUES ";
        $links_data = array();
        $place_holders = array();
        foreach($links as $link){
            array_push (
            ...
            $place_holders [] = "('%d', '%d', '%s', '%s', '%s', '%s', '%s', '%d', '%d', '%s', '%s', '%d')";
        }
